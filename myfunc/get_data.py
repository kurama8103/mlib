...
def get_data_quandl(code, start=None, end=None):
    ...
    if start == None and end == None:
        ...
        df = pd.DataFrame(df_tmp[0], columns=df_tmp[1]
...
def get_ETFs(from_date, to_date):
    ...
    df_etf = dict()
    from_date, to_date = pd.to_datetime(
        [from_date, to_date]).strftime('%d/%m/%Y')
    for k, v in etf_dict.items():
        df_etf[k] = investpy.get_etf_historical_data(etf=v,  country='united states',
                                                     from_date=from_date, to_date=to_date, interval='Daily')['Close']
    return df_etf
